...
def get_pairs_most_freq(content):
    pairs = collections.defaultdict(int)
    for line in content.splitlines():
        prevWord = None
        for word in line.split():
            if prevWord not in [None,'♫','“','”','&amp;',',','.','-','?',')','(','quot',';','--',':','&quot;','quot','và','nhưng','/','tôi','bạn'] and prevWord.isdigit()== False:
                if word not in [None,'♫','“','”','&amp;',',','.','-','?',')','(','quot',';','--',':','&quot;','quot','và','nhưng','/','tôi','bạn'] and prevWord.isdigit()== False:
                    pairs[prevWord, word] += 1
            prevWord = word
    # f.close()
    pairs=dict((k,v) for k,v in pairs.items() if v > 2)
...
def update_pairs(best,content,fvocab):
    ...
    thread1 = Thread_Write_Vocab_Code(fvocab,wordcat)
    thread1.start()
    # print(replaced)
...
def main(infile, outfile, codefile, logfile, num_symbols,  min_frequency=2, verbose=False, is_dict=False):
    ...
    thread1.start()
    # number_symbols is the number get pairs
    for i in range(num_symbols):
        pairs = get_pairs_most_freq(content)
        # pairs1=dict(sorted(pairs.items(), key=operator.itemgetter(1),reverse=True))
        pairs1=dict((v,k) for k,v in pairs.items())
        for v,k in sorted(pairs1.items(),reverse=True):
            # print(k,v)
            content1=""
            content1=update_pairs(k,content,codefile)
            content=content1
            opera = opera +1
            thread = Thread_Print(0,2,k,v,logfile)
            thread.start()
            if opera % 2000 == 0:
...
if __name__ == '__main__':
    ...
    main(args.input, args.output, args.codefile,args.logfile, args.symbols, args.min_frequency, args.verbose, is_dict=args.dict_input)
